...
function delcheck(){
	...
	var url = "deldept";
	var params = "deptno=${deptno}&passwd="+passwd;
	$.ajax({
		type:"post"		//응답 방식	
		,url:url		//요청url = 기존경로 +
		,data:params
		...
		.done(function(args){	//응답이 성공 상태 코드를 반환하면 호출되는 함수//가져온 데이터를 첫번째 매개변수에 넣어줌
			if(args=="삭제처리 되었습니다!"){
				alert(args);
				opener.location.href = "http://192.168.0.43:8083/HRMProjectMain/jsp/dept/deptlist?pageNum=1";
...
<i class="fas fa-fw"></i>
관리자만 삭제 가능합니다.<br> 보안을 위해 관리자 비밀번호를 한번 더 입력 해주세요
</div>
