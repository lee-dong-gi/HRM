...
<head>
  ...
  <title>HRM Project Net - 부서관리</title>
 ...
 $(function(){
	...
	$.ajax({
		...
		.done(function(args){	//응답이 성공 상태 코드를 반환하면 호출되는 함수//가져온 데이터를 첫번째 매개변수에 넣어줌
			 ...
			 if(<%=pageStart-1%> > 0){
			 $("#pageCo").append(
						"<a href='/HRMProjectMain/jsp/dept/deptlist?pageNum="+<%=pageStart-1%>+"'>[이전]</a>"
						 );
			 }
			 for(var i=<%=pageStart%>; i<=<%=pageEnd%>; i++){
				 $("#pageCo").append(
						"<a href='/HRMProjectMain/jsp/dept/deptlist?pageNum="+i+"'>["+i+"]</a>"
						 );
				 }
			 if(<%=pageEnd+1%> < <%=pageCount%>){
			 $("#pageCo").append(
						"<a href='/HRMProjectMain/jsp/dept/deptlist?pageNum="+<%=pageEnd+1%>+"'>[다음]</a>"
						 );
				 }
 			})
	    .fail(function(e) {
      ...
      <div id="content">
        ...
        <div class="container-fluid">
          ...
          <div class="d-sm-flex align-items-center justify-content-between mb-4">
            <h1 class="h3 mb-0 text-gray-800">부서</h1>
          </div>
		<hr class="sidebar-divider">
       	<!-- Content Row -->
          <div class="card shadow mb-4">
            <div class="card-header py-3">
              <h6 class="m-0 font-weight-bold text-primary">조회된 부서 ${count}개</h6>
            </div>
            <div class="card-body">
